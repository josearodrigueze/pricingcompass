(function (window) {
	...
	function Controller(model, view) {
        ...
        that.view.bind('doneItem', function (item) {
            // console.log('soy handler');
            that.doneItem(item.id);
        });
	}
    ...
    /**
     * Marca como realizada o no la tarea, de acuerdo a la variable active.
     *
     * @param {number} id The ID of the item to remove from the DOM and
     * storage
     */
    Controller.prototype.doneItem = function (id) {
        // console.log('Soy la función que llama el handler');
        var that = this;

        that.model.read(id, function (data) {
            data = data[0];
            data.active = !!(data.hasOwnProperty('active') && data.active == false);

            that.model.update(data.id, data, function () {
                that.view.render('doneItem', data);
            });
        });
